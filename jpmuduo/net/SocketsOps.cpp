// 对照原始 muduo SocketsOps.cc 改写：
//   - namespace muduo::net::sockets -> jpmuduo::sockets
//   - jpmuduo 仅支持 IPv4：去掉 sockaddr_in6 分支、VALGRIND 兼容分支
//   - Endian 转换直接使用 htons/ntohs（jpmuduo 无 Endian.h）
//   - 原始 accept 的错误分类处理（预期错误返回 -1、意外错误 FATAL）保留
